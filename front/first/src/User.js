...
const [sortOrder, setSortOrder] = useState("none");
  ...
  doc.text("Amazon Users Data", 14, 10);

  const tableColumn = ["ID", "Name", "Phone", "Address"];
  const tableRows = [];

  users.forEach((u) => {
    tableRows.push([u.ID, u.Name, u.Phone, u.Address]);
  });
...
function sortByName() {
  ...
  // 1️⃣ FIRST CLICK → ASCENDING
  if (sortOrder === "none" || sortOrder === "desc") {
    sorted.sort((a, b) => a.Name.localeCompare(b.Name));   // ASC
    setSortOrder("asc");
  }

  
  else if (sortOrder === "asc") {
    sorted.sort((a, b) => b.Name.localeCompare(a.Name));   // DESC
    setSortOrder("desc");
  }
      ...
      <div style={styles.card}>
        ...
        <div style={styles.btnContainer}>
          ...
          <button onClick={deleteAllData} style={styles.deleteAllBtn}>
            ...
            <button onClick={downloadPDF} style={styles.showBtn}>
            📄 Download PDF
          </button>
        </div>
      </div>
